---
id: home
title: Co-Op Introduction
slug: /
---

Intealth has a long history of working with Drexel co-ops, and several staff members are Drexel alumni.

> Check back here often as this welcome packet gets updated with new content.

## Modern App Team Intro

Learn about the co-op experience at Intealth.

- [🏠 Introduction to Intealth](./introduction-to-intealth)
- [🤖 Digital Transformation](./digital-transformations)
- [🎯 Expectations](./expectations)
- [🖥️ Modern Apps](./modern-apps)
- [📚 CO-OP Alumni](./coop-alumni)

## Getting Started

You can start on this before joining if you want.

- [🔐 Create Accounts](./create-accounts)
- [💻 Tech Stack Learning](./tech-stack-learning)
- [💡 Tech Concept Learning](./tech-concept-learning)
- [📁 Sample Apps](./sample-apps)

## After You've Joined

Get everything set up.

- [💻 Macbook Pro Setup](./macbook-pro-setup)